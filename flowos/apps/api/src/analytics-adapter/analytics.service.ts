...
export class AnalyticsService {
  ...
  async createProject(actor: AnalyticsActor, name: string, description?: string): Promise<AnalyticsProject> {
    ...
    const project = await this.metadata.createProject(actor.workspaceId, trimmedName, description?.trim());
    await this.metadata.createProjectMembership(project.id, actor.id);
    await this.metadata.ensureRetentionDefaults(project.id);
    await this.audit(actor, project.id, "project.created", "project", project.id, { name: project.name });
    return project;
  }

  async uploadCsv(actor: AnalyticsActor, projectId: string, datasetName: string, file: UploadedCsv | undefined): Promise<DatasetVersion> {
    ...
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    ...
    const path = buildAnalyticsObjectPath(actor.workspaceId, projectId, "raw", versionId, file.originalname);
    ...
    try {
      ...
      await this.audit(actor, projectId, "dataset.uploaded", "dataset_version", version.id, { datasetId: version.datasetId, byteSize: version.byteSize, rowCount: profile.dataRowCount });
      return version;
    } catch (error) {
  ...
  async listDatasetVersions(actor: AnalyticsActor, projectId: string): Promise<DatasetVersion[]> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listProfiledDatasetVersions(projectId);
  }

  async createPipeline(actor: AnalyticsActor, projectId: string, name: string, description: string | undefined, definition: AnalyticsPipelineDefinition, isApproved: boolean): Promise<AnalyticsPipelineVersion> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    const trimmedName = name.trim();
    ...
    await this.audit(actor, projectId, "pipeline.version_created", "pipeline_version", version.id, { isApproved: version.isApproved, nodeCount: version.definition.nodes.length });
    return version;
  }

  async listPipelineVersions(actor: AnalyticsActor, projectId: string): Promise<AnalyticsPipelineVersion[]> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listPipelineVersions(projectId);
  }

  async listPipelineRuns(actor: AnalyticsActor, projectId: string): Promise<AnalyticsPipelineRun[]> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listPipelineRuns(projectId);
  }

  async listQualityReports(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listQualityReports(projectId);
  }

  async listModelVersions(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listModelVersions(projectId);
  }

  async listModelEvaluations(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listModelEvaluations(projectId);
  }

  async approveModel(actor: AnalyticsActor, projectId: string, modelVersionId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    const model = await this.metadata.approveModel(projectId, modelVersionId);
    await this.audit(actor, projectId, "model.approved", "model_version", model.id, { modelFamily: model.modelFamily, wape: model.metrics.wape });
    return model;
  }

  async trainModel(actor: AnalyticsActor, projectId: string, request: ModelTrainingRequest) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    ...
    const artifact = { bucket: "analytics-model", path: buildAnalyticsObjectPath(actor.workspaceId, projectId, "model", modelId, "model.json"), artifactKind: "model" as const };
    ...
    try {
      const result = await this.runWorkerJob(jobId, () => this.worker.trainModel({
      ...
      // A worker can establish quality eligibility, but only a role-authorized person approves a model.
      await this.metadata.markModelTrainingSucceeded({ projectId, modelId, jobId, trainingDatasetVersionId: trainingVersion.id, artifact: result.modelArtifact, modelFamily: result.modelFamily, featureSet: result.featureSet, metrics: result.metrics, dataFingerprint: result.dataFingerprint, isApproved: false, evaluations: result.evaluations });
      await this.metadata.createInitialDriftReport(projectId, modelId, trainingVersion.id, result.dataFingerprint);
      const model = { ...pending, artifact: result.modelArtifact, modelFamily: result.modelFamily, metrics: result.metrics, dataFingerprint: result.dataFingerprint, isApproved: false, status: "succeeded" as const };
      await this.audit(actor, projectId, "model.training_completed", "model_version", model.id, { modelFamily: result.modelFamily, qualityEligible: result.isApproved, wape: result.metrics.wape });
      return model;
    } catch (error) {
  ...
  async listPredictionRuns(actor: AnalyticsActor, projectId: string): Promise<PredictionContract[]> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listPredictionRuns(projectId);
  }

  async listAuditEvents(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listAuditEvents(projectId);
  }

  async listRetentionPolicies(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listRetentionPolicies(projectId);
  }

  async updateRetentionPolicy(actor: AnalyticsActor, projectId: string, artifactKind: "raw" | "processed" | "model" | "prediction", retentionDays: number) {
    if (!Number.isInteger(retentionDays) || retentionDays < 1 || retentionDays > 3650) throw new BadRequestException("Retention days must be between 1 and 3650.");
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    const policy = await this.metadata.updateRetentionPolicy(projectId, artifactKind, retentionDays);
    await this.audit(actor, projectId, "retention.updated", "retention_policy", artifactKind, { retentionDays, cleanupWorkerDeployed: false });
    return policy;
  }

  async listOperations(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listOperations(projectId);
  }

  async listDriftReports(actor: AnalyticsActor, projectId: string) {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    return this.metadata.listDriftReports(projectId);
  }

  async resolvePredictionSummaryReference(actor: AnalyticsActor, reference: AnalyticsResultReference): Promise<AnalyticsPredictionSummaryView> {
    ...
    await this.metadata.assertProjectInWorkspace(reference.projectId, actor.workspaceId);
    ...
    await this.audit(actor, reference.projectId, "prediction.summary_viewed", "prediction_run", prediction.id, { modelVersionId: prediction.modelVersionId, rowCount: prediction.summary.rowCount });
  ...
  async predict(actor: AnalyticsActor, projectId: string, modelVersionId: string, input: PredictionInput): Promise<PredictionContract> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    ...
    const jobId = randomUUID();
    const artifact = { bucket: "analytics-prediction", path: buildAnalyticsObjectPath(actor.workspaceId, projectId, "prediction", runId, "predictions.csv"), artifactKind: "prediction" as const };
    ...
    try {
      const result = await this.runWorkerJob(jobId, () => this.worker.predict({
      ...
      await this.audit(actor, projectId, "prediction.completed", "prediction_run", prediction.id, { modelVersionId, mode: input.mode, rowCount: result.summary.rowCount });
      return prediction;
    } catch (error) {
  ...
  async runPipeline(actor: AnalyticsActor, projectId: string, pipelineVersionId: string, sources: PipelineSourceBinding[], outputDatasetName?: string): Promise<AnalyticsPipelineRun> {
    await this.metadata.assertProjectInWorkspace(projectId, actor.workspaceId);
    ...
    const outputPath = buildAnalyticsObjectPath(actor.workspaceId, projectId, "processed", outputVersionId, "processed.csv");
    ...
    try {
      const result = await this.runWorkerJob(workerJobId, () => this.worker.processDataset({
      ...
      await this.audit(actor, projectId, "pipeline.completed", "pipeline_run", completed.id, { pipelineVersionId, outputDatasetVersionId: outputVersion.id, outputRows: result.qualityReport.outputRowCount });
      return completed;
    } catch (error) {
  ...
  private audit(actor: AnalyticsActor, projectId: string, action: string, resourceType: string, resourceId: string, details: Record<string, unknown>): Promise<void> {
    return this.metadata.recordAuditEvent({ workspaceId: actor.workspaceId, projectId, action, resourceType, resourceId, actorId: actor.id, details });
  }

  private async runWorkerJob<T>(jobId: string, operation: () => Promise<T>): Promise<T> {
    let lastError: unknown;
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      await this.metadata.markJobAttempt(jobId);
      try { return await operation(); }
      catch (error) {
        lastError = error;
        if (attempt < 2) await this.metadata.markJobRetrying(jobId, error instanceof Error ? error.message : "Analytics worker failed.");
      }
    }
    throw lastError;
  }
